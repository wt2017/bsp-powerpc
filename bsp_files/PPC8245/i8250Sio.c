...
--------------------
2005-11-30 , xiexy change , walk around a special problem(transmit interupt 
		   loss) of the i8250 int service program; 
...
#       define  I8250_DEFAULT_BAUD      19200
#endif
...
LOCAL STATUS i8250OptsSet
    ...
    char ier = (*pChan->inByte) (pChan->ier);
    char mask;
...
void i8250Int
 ...
 //   do {   //xie commit this for the problem(transmit interupt loss)

	interruptID &= I8250_IIR_MASK;
...
/*        interruptID = (*pChan->inByte) (pChan->iid);//xie commit this for the problem(transmit interupt loss)
  
	} while (((interruptID & 0x01) == 0) && (ix++ < 10)); //xie commit this for the problem(transmit interupt loss)
*/	
    }
...
LOCAL int i8250PRxChar
    ...
    char pollStatus = ( *(pChan)->inByte) (pChan->lst);
...
static I8250_CHAN  i8250Chan[N_SIO_CHANNELS];
	
#define UART_REG(reg, chan) (SERIAL_BASE(chan) + reg)
	

void sysSerialBaudRateUpdate(int rate, int *newRegSetting);
...
{
	int i = 10000;
	while (!(sysSerialInByte(SERIAL_BASE(0)+5) & 0x40) && i--)
		;
	sysSerialOutByte(SERIAL_BASE(0), c);
}

int sysSerialGetc(void)
{
	while (!(sysSerialInByte(SERIAL_BASE(0)+5) & 0x01))
		;
	return sysSerialInByte(SERIAL_BASE(0));
}
...
void sysSerialPrintHex(UINT32 value, int cr)
{
	...
	il = intLock();
	sysSerialOutByte(SERIAL_BASE(0)+3, 0x83);
	sysSerialOutByte(SERIAL_BASE(0)+0, 0xd9);	/* 19200 bps */
	sysSerialOutByte(SERIAL_BASE(0)+1, 0x00);	 
	sysSerialOutByte(SERIAL_BASE(0)+3, 0x03);
	sysSerialOutByte(SERIAL_BASE(0)+4, 0x02);
	for (i = 0; i < 8; i++)
...
void sysSerialHwInit (void)
{
	...
	i8250Chan[i].int_vec	 = EPIC_VECTOR_UART1 + i;
	...
	i8250Chan[i].outByte	 = (void (*) (int, char)) sysSerialOutByte;
	i8250Chan[i].inByte  = (UCHAR (*) ()) sysSerialInByte;
...
void sysSerialHwInit2 (void)
{
	...
	/* now to connect the interrupt service routine to the vector */
	for (i=0; i<N_SIO_CHANNELS; i++)
	{
		intConnect (INUM_TO_IVEC ((int ) i8250Chan[i].int_vec),
			   (VOIDFUNCPTR)i8250Int, (int)&i8250Chan[i] );

		intEnable(i8250Chan[i].int_vec);
	}
...
SIO_CHAN * 
sysSerialChanGet(int channel)		/* serial channel */
{
