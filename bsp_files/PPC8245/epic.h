...
#define EPIC_TM_CONTROL_REG	(EPIC_EUMBBAR + 0x010f4)/* Timer Frequency */

/* 2.Global timer Interrupt (vector/priority)/destination Registers */
...
/* 3.IRQ Interrupt (vector/priority)/destination Registers */
...
/* 4.Serial Interrupt (vector/priority)/destination Registers */
...
/* 5.I2C,DMA,Msg Unit Interrupt (vector/priority)/destination Registers */
...
/* 6.DUART interrupt (vector/priority)/destination Registers */
...
#define	EPIC_VECREG_PRIORITY(x)	(x<<16)	/* Priority */

/*if use serial interrupt,the vector can be continuous*/
