...
STATUS sysMotTsecEnetAddrGet
    ...
    UCHAR * pAddr   /* where to copy the Ethernet address */
    )
    {
	...
	if(unit == 2)
	{
		...
		{
			bsp_ifadrs_get_mac(0 , pAddr);
		}
		else                          /*使用默认的MAC地址*/
		{
			memcpy(pAddr , bsp_default_cpm_enet_addr , 6);
		}
	}
	else if(unit == 0)
	{
		...
		else if(bsp_ifadrs_check(2) == 0) 
		{
			bsp_ifadrs_get_mac(2 , pAddr);
		}
		else						  /*使用默认的MAC地址*/
		{
			memcpy(pAddr , bsp_default_cpm_enet_addr , 6);
