...
warm:
	...
	bl     resetEntry
start:	
		cmpwi   r10, BOOT_NORMAL
		beq     warmstart
		cmpwi   r10, BOOT_NO_AUTOBOOT
		beq     warmstart
		cmpwi   r10, BOOT_CLEAR
		beq     warmstart
		cmpwi   r10, BOOT_QUICK_AUTOBOOT
		beq     warmstart
		li      r10, BOOT_COLD
warmstart:
...
/*点灯测试代码*/
   	lis 	r24, HIADJ (M85XX_GPIOCR)	
	addi	r24, r24, LO (M85XX_GPIOCR)
   	lis 	r22, HIADJ (0x00000200)	
	addi	r22, r22, LO (0x00000200)
	stw 	r22, 0(r24)

	lis		r11, HIADJ (0x3E80000)		/* set the time	counter.*/
	addi	r11, r11, LO (0x3E80000)
	
   	lis 	r23, HIADJ (BSP_LIGHT_REG_1)	
	addi	r23, r23, LO (BSP_LIGHT_REG_1)
	lis	    r22, HIADJ (BSP_LED_ALL_1)	/*灭灯*/	
...
FUNC_LABEL(_resetEntry)
	...
	mtspr L1CSR1, r6        /* temp disabled Enable the Instruction cache*/
        ...
        isync
        li	r7, 0x0000    /*zcw enable?  应该是0x0001吧*/
	msync
	isync
	mtspr	L1CSR1, r7		/* enable the instruction cache */
...
/*DBSR bits are cleared by writing ones to them; writing zeros has no effect.*/
        mfspr r1, DBSR
        mtspr DBSR, r1
	 ...
	 * 256MB
        ...
        ori    r4,r4,0x0000
        mtspr  MAS0, r4				/*zcw Writing to MAS0 requires synchronization?*/
        addis  r5,0,0xc000           /* V = 1, IPROT = 1, TID = 0*/
        ori    r5,r5,_MMU_TLB_SZ_64M  /* TS = 0, TSIZE = 64 MByte page size*/
        mtspr  MAS1, r5
        addis  r6,0,0xfc00           /* EPN = 0xfc000000 zcw 这个值似乎应该需要修改??*/
        ori    r6,r6,0x000a          /* WIMGE = 01010 */
        mtspr  MAS2, r6
        addis  r7,0,0xfc00           /* RPN = 0xf0000000*/
	...
	/*
        ...
        ori    r5,r5,_MMU_TLB_SZ_256M /* TS = 0, TSIZE = 256 MByte page size*/
        mtspr  MAS1, r5
        addis  r6,0,HI(0xa0000000) /* EPN = 0xa0000000 */
        ori    r6,r6,0x0016          /* WIMGE = 01010 */
        mtspr  MAS2, r6

        addis  r7,0,HI(0xa0000000) /* RPN = 0xa0000000 */
        ori    r7,r7,0x003F          /* Supervisor XWR*/
        mtspr  MAS3, r7
        isync
        msync
...
ccsrbarWait:
	...
	/*以下配置cs*/
	/* load BR0 */
	WRITEADR(r6,r7,M85XX_BR0 (CCSBAR),0xfc001001)   /*64M FLASH*/
	/* load OR0 */
	WRITEADR(r6,r7,M85XX_OR0 (CCSBAR),0xfc006f27)
	isync
	sync
	mbar 0
	
	/*zcw 在datasheet中没找到M85XX_DCR0/M85XX_DCR1该寄存器??*/
...
dllDelay4:
	...
	WRITEOFFSET(r6,r7,(DDR_SDRAM_CLK_CTRL), 0x03800000)  /*zcw 怎么搞了2次??? */
	
	WRITEOFFSET(r6,r7,(CS0_BNDS), 0x0000001f)     /*zcw 512M*/ 
	WRITEOFFSET(r6,r7,(CS1_BNDS), 0x00000000)     
	WRITEOFFSET(r6,r7,(CS2_BNDS), 0x00000000)
	WRITEOFFSET(r6,r7,(CS3_BNDS), 0x00000000)
	WRITEOFFSET(r6,r7,(CS0_CONFIG), 0x80C44102)   /* 3 logical bank,13 row,10 column*/
	...
	WRITEOFFSET(r6,r7,(DDR_SDRAM_MODE_CFG_2), 0x40003000)
...
defaultDDRInit:	
	...
	WRITEOFFSET(r6,r7,(DDR_SDRAM_CFG), 0x43008000)   /*no ecc, 64-bit bus*/
	WRITEOFFSET(r6,r7,(DDR_SDRAM_CFG_2), 0x24600000) /*Always keep ODT asserted to internal IOs*/
	WRITEOFFSET(r6,r7,(DDR_SDRAM_MODE_CFG), 0x20400842)  /*??*/
	WRITEOFFSET(r6,r7,(DDR_SDRAM_INTERVAL), 0x08200100)
	WRITEOFFSET(r6,r7,(0xf08), 0x0000200)  /*zcw 找不到该寄存器*/
...
finalDDRInit:			
	lis    r4,HI(CCSBAR|DDR_IO_OVCR)   /*zcw 找不到该寄存器*/
...
SdramWrLoop2:
	 ...
	 * 16MB
        ...
        mtspr  MAS1, r5
        addis  r6,0,0xfc00           /* EPN = 0xfc000000*/
...
#if 0
/*单字节检查*/
bsp_check_ddr_sdram_start:

	/*prepare to check the sdram. */

	lis		r15, HIADJ (0x5a5a5a5a)
	addi	r15, r15, LO (0x5a5a5a5a)

	lis		r6,	HIADJ (LOCAL_MEM_LOCAL_ADRS + LOCAL_MEM_SIZE - 4)	/* set the start address. */
	addi	r6,	r6,	LO (LOCAL_MEM_LOCAL_ADRS + LOCAL_MEM_SIZE - 4)
