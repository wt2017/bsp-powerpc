...
Descriptors and a non-configurable number of transmit buffers, i.e.
...
LOCAL STATUS    motTsecEndStatsDump (TSEC_DRV_CTRL * pDrvCtrl);
...
LOCAL STATUS    motTsecMemFree (TSEC_DRV_CTRL * pDrvCtrl);
...
#if 0
LOCAL UINT32 motTsecIvecToInum(TSEC_DRV_CTRL *pDrvCtrl,UINT32 vec)          
    {                                          
	if (pDrvCtrl->ivecToInum == NULL)      
	    return (vec);                        
        else                                   
            return(pDrvCtrl->ivecToInum(vec));    
    };
#define MOT_TSEC_IVEC_TO_INUM(x,y)  motTsecIvecToInum(x,y)
#endif
 ...
 * instances.
 */
LOCAL const int tsecVec[4][3] = {
{EPIC_TSEC1TX_INT_VEC, EPIC_TSEC1RX_INT_VEC,EPIC_TSEC1ERR_INT_VEC},
{EPIC_TSEC2TX_INT_VEC, EPIC_TSEC2RX_INT_VEC,EPIC_TSEC2ERR_INT_VEC},
{EPIC_TSEC3TX_INT_VEC, EPIC_TSEC3RX_INT_VEC,EPIC_TSEC3ERR_INT_VEC},
{EPIC_TSEC4TX_INT_VEC, EPIC_TSEC4RX_INT_VEC,EPIC_TSEC4ERR_INT_VEC}};
...
/*注意，我们这里使用TSEC0和TSEC2*/
    ...
    bcopy ((char *) pDrvCtrl->enetAddr.octet,
    ...
    MOT_TSEC_LOG (MOT_TSEC_DBG_ANY, "MAC %02x:%02x:%02x:%02x:%02x:%02x\n",
                  pDrvCtrl->enetAddr.octet[0],
                  pDrvCtrl->enetAddr.octet[1],
                  pDrvCtrl->enetAddr.octet[2],
                  pDrvCtrl->enetAddr.octet[3],
                  pDrvCtrl->enetAddr.octet[4],
                  pDrvCtrl->enetAddr.octet[5]
                  );
...
LOCAL STATUS motTsecUnload
    ...
    pDrvCtrl->state = MOT_TSEC_STATE_NOT_LOADED;
...
/*******************************************************************************
*
* motTsecMemFree - Free the driver's memory
*
*
* RETURNS: OK 
*/

LOCAL STATUS motTsecMemFree
    (
    TSEC_DRV_CTRL *pDrvCtrl       /* pointer to DRV_CTRL structure */
    )
    {
    int         ix;
    M_BLK_ID    pMblk;
...
#ifdef USE_NET_JOB_ADD
#define HANDLERS_BUSY (pDrvCtrl->rxJobQued || pDrvCtrl->txHandlerQued)
#else
#define HANDLERS_BUSY \
    (QJOB_IS_BUSY (&pDrvCtrl->txJob) || QJOB_IS_BUSY (&pDrvCtrl->rxJob))
#endif
    ...
    if (HANDLERS_BUSY)
        ...
        if (pDrvCtrl->pMblkList)
            ...
            cfree  ((char *)pDrvCtrl->pMblkList);
            pDrvCtrl->pMblkList = NULL;
            }

        if ((MOT_TSEC_FLAG_SET (MOT_TSEC_OWN_BD_MEM)) &&
           (pDrvCtrl->pBdAlloc != NULL))
            {
    ...
    free (pDrvCtrl->stats);
        ...
        {
        if ((NET_JOB_ADD ((FUNCPTR) motTsecMemFree, (int) pDrvCtrl,
                        0,0,0,0)) == ERROR)
            {
...
LOCAL STATUS motTsecInitParse
   ...
   /* MAC Address xx-xx-xx-xx-xx-xx:*/
    ...
    for (i = 0; i < sizeof (pDrvCtrl->enetAddr.octet)-1; i++)
        {
        tok = strtok_r (NULL, "-", &holder);
        if (tok == NULL)
          return ERROR;

        pDrvCtrl->enetAddr.octet[i] = (int) strtoul (tok, NULL, 16);
        }
...
LOCAL STATUS motTsecFuncInit
    ...
    if (pDrvCtrl->initFuncs != NULL)
        ...
        pDrvCtrl->extWriteL2AllocFunc   = pDrvCtrl->initFuncs->extWriteL2AllocFunc;
	...
	{
	/* Default to 8540 TSEC - Assumes unit 0 and 1 are TSEC 1 & 2 
	 * respectively. 
	 */
	pDrvCtrl->inum_tsecTx = tsecVec[pDrvCtrl->unit][0];
	pDrvCtrl->inum_tsecRx = tsecVec[pDrvCtrl->unit][1];
	pDrvCtrl->inum_tsecErr = tsecVec[pDrvCtrl->unit][2];
...
LOCAL STATUS motTsecParmInit
    ...
    switch ((UINT32)(pDrvCtrl->userFlags & MOT_TSEC_USR_MODE_MASK))
        ...
        case MOT_TSEC_USR_MODE_MII:
            ...
            motTsecMiiPhyRead (pDrvCtrl, pDrvCtrl->phyInfo->phyAddr, 0x0, 
                               &phyData);
            phyData |= 0x1000;
            motTsecMiiPhyWrite (pDrvCtrl, pDrvCtrl->phyInfo->phyAddr, 0x0, 
                                phyData);
...
LOCAL STATUS motTsecPhyParmInit
    ...
    if (pDrvCtrl->phyInitFunc != NULL && pDrvCtrl->phyInit != NULL)
        ...
        pDrvCtrl->phyFlags              |= MII_PHY_PRE_INIT | MII_PHY_100 |
                                           MII_PHY_AUTO     | MII_PHY_10  | 
                                           MII_PHY_HD       | MII_PHY_FD  |
                                           MII_PHY_1000T_FD | MII_PHY_DEF_SET;
        ...
        pDrvCtrl->phyInfo->phyAnOrderTbl = NULL;
...
/* number of different clusters sizes in pool -- only 1 */
    ...
    if (pDrvCtrl->initParms)
        {
        if ((pDrvCtrl->pBufAlloc = pDrvCtrl->initParms->memBufPtr) != NULL)
            {
    ...
    if (pDrvCtrl->initType == END_NET_POOL_INIT)
        ...
        clDescTbl[0].memSize =
            (clDescTbl[0].clNum * clDescTbl[0].clSize) + 256;
    ...
    if ((pDrvCtrl->pClPoolId = netClPoolIdGet (pDrvCtrl->endObj.pNetPool, 
                               MOT_TSEC_MAX_CL_LEN, FALSE)) == NULL )
        return ERROR;
...
STATUS motTsecStart
    ...
    if (pDrvCtrl->phyInitFunc != NULL)
        ...
        if ((pDrvCtrl->phyInitFunc (pDrvCtrl->phyInfo)) != OK)
            ...
            for (ix = 0;  ix < 5; ix++)
                ...
                motTsecMiiPhyRead (pDrvCtrl,pDrvCtrl->phyInit->phyAddr,
                                   MOT_TSEC_PHY_GIG_STATUS_REG,
                                   &value);

		/* if phy is returning all ffff it's not a valid value */
		if(value!=0xffff)
		    {
		...
		/* Give the auto neg a chance */
		MOT_TSEC_MS_DELAY(20);
...
LOCAL void motTsecRxInt
    ...
    if (status & (MOT_TSEC_IEVENT_RXF0 | 
                  MOT_TSEC_IEVENT_BSY))
        {
...
LOCAL void motTsecTxInt
    ...
    /* clear pending events */
    MOT_TSEC_IEVENT_REG = event & (MOT_TSEC_IEVENT_TXF | MOT_TSEC_IEVENT_TXE);
    ...
    if (( status & (MOT_TSEC_IEVENT_TXF  | 
                    MOT_TSEC_IEVENT_TXE  | 
                    MOT_TSEC_IEVENT_BABT | 
                    MOT_TSEC_IEVENT_XFUN)) !=0)
        {
...
void motTsecDumpTxRing
    ...
    index = pDrvCtrl->tbdIndex;
    cleanx = pDrvCtrl->tbdCleanIndex;

    printf("motTsecDumpTxRing: Current TBD index %d at %p status 0x%x\n",
	   index, &pDrvCtrl->pTbdBase[index],
	   pDrvCtrl->pTbdBase[index].bdStat);
    printf ("  tbdCleanIndex %d at %p status 0x%x\n",
	    cleanx, &pDrvCtrl->pTbdBase[cleanx],
	    pDrvCtrl->pTbdBase[cleanx].bdStat);

    for (i = 0; i < pDrvCtrl->tbdNum; i++)
        {
	pTestTbd = &pDrvCtrl->pTbdBase[i];

        printf("  index %d pTxDescriptor %p status 0x%x len %d data 0x%x\n", 
               i, pTestTbd, pTestTbd->bdStat, pTestTbd->bdLen,
	       pTestTbd->bdAddr);
        }
    }
...
LOCAL STATUS motTsecSend
    ...
    if ( pMblk == NULL )
        goto motTsecSend_inval;

    END_TX_SEM_TAKE(&pDrvCtrl->endObj, WAIT_FOREVER);

    if (pDrvCtrl->tbdFree == 0)
        goto motTsecSend_block;

    /*
     * We must access tbdIndex inside TX mutual exclusion only. The others
     * are likely stable.
     */

    pTbdBase = pDrvCtrl->pTbdBase;
    tBufList = pDrvCtrl->tBufList;
    mask = pDrvCtrl->tbdMask;
    index = pDrvCtrl->tbdIndex;
...
oneFrag:
        ...
        MOT_TSEC_CACHE_FLUSH (pMblk->mBlkHdr.mData, pMblk->mBlkHdr.mLen);
   ...
   /* check we have enough resources */
    ...
    if (fragNum <= pDrvCtrl->tbdFree &&
	fragNum < MOT_TSEC_FRAG_NUM_LIMIT)
        {
        /* save the first TBD index */
        first = index;

        /* caculate the last fragments index */
        ...
        bdStat = MOT_TSEC_TBD_I; /* for first TBD */

        for ( ; fragNum; pCurr = (M_BLK *)pCurr->mBlkHdr.mNext)
            {
	    len = pCurr->mBlkHdr.mLen;
	    if (len == 0)
		continue;
		...
		++index;
            ...
            /* Set the ready bit for subsequent TBDs. */

	    bdStat = MOT_TSEC_TBD_R | MOT_TSEC_TBD_I;
        ...
        /*
	 * As the above code has already set I and W appropriately
         * for the first fragment, we only need to OR in R.
	 */

	pTbdBase[first].bdStat |= MOT_TSEC_TBD_R;

        }
    else
        {
	/* We know there is at least one free TBD. Coalesce the packet. */

	if ((pMblk = netTupleGet (pDrvCtrl->endObj.pNetPool,
	...
	len = netMblkToBufCopy (pCurr, pMblk->mBlkHdr.mData, NULL);

	netMblkClChainFree (pCurr);

	pMblk->mBlkPktHdr.len = pMblk->mBlkHdr.mLen = len;
...
LOCAL STATUS motTsecTbdInit
    ...
    if (pDrvCtrl->pTxPollMblk == NULL)
        ...
        pDrvCtrl->pTxPollBuf = pMblk->mBlkHdr.mData;
        }
...
/******************************************************************************
*
* motTsecDumpRxRing - Show the Receive Ring details
*
* This routine displays the transmit ring descriptors.
*
* SEE ALSO: motTsecDumpRxRing()
...
void motTsecDumpRxRing
    ...
    index = pDrvCtrl->rbdIndex;

    pTestRbd = &pDrvCtrl->pRbdBase[index];

    printf("motTsecDumpRxRing: Current RBD index %d at %p status 0x%x\n",
            index, pTestRbd, pTestRbd->bdStat);

    for (i = 0; i < pDrvCtrl->rbdNum; i++)
        {
	pTestRbd = &pDrvCtrl->pRbdBase[i];

        printf("  index %d pRxDescriptor %p status 0x%x len %d addr 0x%x\n", 
               i, pTestRbd, pTestRbd->bdStat, pTestRbd->bdLen,
	       pTestRbd->bdAddr);
        }
    }
...
LOCAL void motTsecHandleRXFrames
    ...
    while (TRUE)
	{
	bdStat = rbd[index].bdStat;

	if (((bdStat & MOT_TSEC_RBD_E)!=0)  || (--loopCounter == 0))
	  break;
        ...
        if ((pNewMblk = netTupleGet (pDrvCtrl->endObj.pNetPool, 
                        MOT_TSEC_CL_SIZE, M_DONTWAIT, MT_DATA, 0)) == NULL)
            {
            ...
            /* Pass RBD ownwership to the TSEC */

            rbd[index].bdStat = bdStat | MOT_TSEC_RBD_E;
	 ...
	 * 
	 * Note, this must be done BEFORE we check the E bit on the current
	 * descriptor. The old code cleared the RXF0 event AFTER checking
	 * the E bit.  Suppose that with the old code, a frame came in right
	 * after we had found the E bit set, and before we cleared the RXF0
	 * event. In that case (if pollCnt has reached pollLoops), the driver
	 * wouldn't handle that frame until another packet arrives, which
	 * could be significantly later.  This was seen in the Netperf TCP_RR
	 * test.
	 *
	 * The eieio (or something like it) is necessary.
	 *
	 * We also want to avoid reading bdStat more than once.
	 */
	...
	CACHE_PIPE_FLUSH();
#else
	WRS_ASM ("eieio"); /* gnu assembler doesn't recognize msync/mbar */
#endif

	if ((rbd[pDrvCtrl->rbdIndex].bdStat & MOT_TSEC_RBD_E) == 0)
	    goto again;
...
LOCAL STATUS motTsecAddrSet
    ...
    MOT_TSEC_MACSTNADDR2_REG =
        ((int)pAddr[1]<<24) + ((int)pAddr[0]<<16);
...
LOCAL STATUS motTsecIoctl
    ...
    switch ((UINT32) cmd)
		...
		MOT_TSEC_FLAG_SET (MOT_TSEC_PROM);
		    ...
		    MOT_TSEC_RCTRL_REG |= MOT_TSEC_RCTRL_PROM;
                    /* config up */
...
/***************************************************************************
*
* motTsecEndStatsDump - Dump statistic registers for MIB2 (RFC2233)
*
...
LOCAL STATUS motTsecEndStatsDump
    (
    TSEC_DRV_CTRL *  pDrvCtrl    /* device receiving command */
    )
    {
    ...
    val = MOT_TSEC_RBYT_REG;
    pEndStatsCounters->ifInOctets = val; 

    /* Get number of TX'ed octets */
    
    val = MOT_TSEC_TBYT_REG;
    pEndStatsCounters->ifOutOctets = val; 

    /* Get RX'ed unicasts, broadcasts, multicasts */

    val = MOT_TSEC_RPKT_REG;
    pEndStatsCounters->ifInUcastPkts = val;

    val = MOT_TSEC_RBCA_REG;
    pEndStatsCounters->ifInBroadcastPkts = val;

    val = MOT_TSEC_RMCA_REG;
    pEndStatsCounters->ifInMulticastPkts = val;

    val = MOT_TSEC_RDRP_REG;
    pEndStatsCounters->ifInDiscards = val;
    ...
    val = MOT_TSEC_TPKT_REG;
    pEndStatsCounters->ifOutUcastPkts = val;    

    val = MOT_TSEC_TBCA_REG;
    pEndStatsCounters->ifOutBroadcastPkts = val;

    val = MOT_TSEC_TMCA_REG;
    pEndStatsCounters->ifOutMulticastPkts = val;
...
LOCAL STATUS motTsecMiiPhyRead 
    ...
    MOT_TSEC_LOG (MOT_TSEC_DBG_MII,"motTsecMiiPhyRead PHY(%d) REG(%d)=0x%04x\n",
                   (int)phyAddr, regAddr, phyValue,4,5,6);
...
LOCAL STATUS motTsecPhyPreInit
    ...
    if ( MOT_TSEC_USR_FLAG_ISSET (MOT_TSEC_USR_PHY_MON))
        ...
        pDrvCtrl->phyInfo->phyFlags &= ~MII_PHY_MONITOR;
#endif
    if ( MOT_TSEC_USR_FLAG_ISSET (MOT_TSEC_USR_MODE_MII))
        {
        pDrvCtrl->phyInfo->phyFlags  = (MII_PHY_PRE_INIT | MII_PHY_100 |
                                        MII_PHY_AUTO     | MII_PHY_10  |
                                        MII_PHY_HD       | MII_PHY_FD  |
                                        MII_PHY_DEF_SET  | MII_PHY_TBL |
                                        MII_PHY_1000T_FD );
        }
...
LOCAL void motTsecHandleLSCJob
    ...
    if (miiStat & MII_SR_LINK_STATUS) /* if link is now up */
        ...
        if (pDrvCtrl->phyStatusFunc)
            ...
            if ( retVal == OK )
                ...
                if ( status.duplex == MOT_TSEC_PHY_DUPLEX_FULL)
                    {
                    val |= MOT_TSEC_MACCFG2_FULL_DUPLEX;
                    duplex = 1;
                    }
                else if ( status.duplex == MOT_TSEC_PHY_DUPLEX_HALF)
                    {
                    val &= ~MOT_TSEC_MACCFG2_FULL_DUPLEX;
                    duplex = 0;
                    }
                ...
                if (status.speed == MOT_TSEC_PHY_SPEED_10)
                    {
                    val |= MOT_TSEC_ECNTRL_R100M;
                    speed = 0;
                    }
                else if (status.speed == MOT_TSEC_PHY_SPEED_100)
                    {
                    val &= ~MOT_TSEC_ECNTRL_R100M;
                    speed = 1;
                    }
                else if (status.speed == MOT_TSEC_PHY_SPEED_1000)
                    {
                    val &= ~MOT_TSEC_ECNTRL_R100M;
                    speed = 2;
                    }
    ...
    MOT_TSEC_LOG ( MOT_TSEC_DBG_MII,"TSEC %d Unit %d Link %s @ %s BPS %s Duplex\n",
                   pDrvCtrl->tsecNum, pDrvCtrl->unit, (int)linkStr[link],
                   (int)speedStr[status.speed],(int)duplexStr[status.duplex], 6);

    intLevel = intLock(); /* Hmm. */
...
/******************************************************************************
*
* motTsecPhyLSCInt - Line Status Interrupt. This int handler adds a job when
...
LOCAL void motTsecPhyLSCInt
    ...
    */
    ...
    if ( !pDrvCtrl->lscHandling )
        ...
        if ( pDrvCtrl->phyStatusFunc )
            {
            if ( NET_JOB_ADD ( (FUNCPTR) motTsecHandleLSCJob,
...
void motTsecMiiShow
    ...
    strcpy ((char *) speed, (pDrvCtrl->phyInfo->phySpeed == 1) ?
                    "100Mbit/s" : "10Mbit/s");
...
/******************************************************************************
*
* motTsecRegsShow - Debug Function to show driver specific contol data.
*
...
/****************************************************************************
*
* motTsecShow - Debug Function to show driver specific contol data.
*
* This function is only available when MOT_TSEC_DBG is defined. 
* It should be used for debugging purposes only.
*
...
void motTsecShow
    ...
    MOT_TSEC_LOG (MOT_TSEC_DBG_TX,
        "\n\tTxBds:Base 0x%x Total %d Free %d Next %d NextToClean %d\n",
        (int)pDrvCtrl->pTbdBase,
        pDrvCtrl->tbdNum,
        pDrvCtrl->tbdFree,
	pDrvCtrl->tbdIndex,
        pDrvCtrl->tbdCleanIndex, 6);
    ...
    MOT_TSEC_LOG (MOT_TSEC_DBG_RX,
        ("\n\tRxBds:Base 0x%x Total %d Next %d\n"),
...
#endif
void BspPhyShow(int tsecNum,unsigned char addr,int num)
{
    TSEC_DRV_CTRL * pDrvCtrl = NULL;
    unsigned char i = 0;
...
void BspPhyRead(int tsecNum,unsigned char addr,unsigned short Reg, unsigned short* pValue)
{
    TSEC_DRV_CTRL * pDrvCtrl = NULL;
    if ((pDrvCtrl = (TSEC_DRV_CTRL *)endFindByName ("mottsec", tsecNum)) 
                     == NULL) 
    ...
    motTsecMiiPhyRead (pDrvCtrl, addr, Reg, pValue);
}
void BspPhyWrite(int tsecNum,unsigned char addr, unsigned short Reg, unsigned short Value)
{
    TSEC_DRV_CTRL * pDrvCtrl = NULL;
    if ((pDrvCtrl = (TSEC_DRV_CTRL *)endFindByName ("mottsec", tsecNum)) 
                     == NULL) 
    ...
    motTsecMiiPhyWrite(pDrvCtrl, addr, Reg, Value);
}
#endif /* MOT_TSEC_DBG */
