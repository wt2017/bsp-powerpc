...
int RamDataLnTst(unsigned int *RamAdr);
int RamAdrLnTst(char *RamStrtAdr,unsigned int RamSize );
...
void romStart
    ...
    unsigned int i = 0 , *LedReg = (unsigned int *)BSP_LIGHT_REG;
...
/* If cold booting, clear memory to avoid parity errors */
    ...
    *LedReg = (BSP_LED_RED|BSP_LED_GREEN|BSP_DUAL_LED_GREEN) ;/*light green light*/
...
{
	unsigned int pattern, outData;

	/* shift 1 on the data line */
...
/* RamAdrLnTst
 *   -- test the Address line
 * INPUT:
 *  RamStrtAdr -- the start address of the SDRAM to be test
 *  RamSize    -- the size of the SDRAM to be test
 * output:
 *  OK or ERROR.
 *  if test error,return error and record the first error.
 */
int RamAdrLnTst
(
	char * RamStrtAdr,
	unsigned int RamSize	/* bytes size */
)
{
	unsigned int offset;
	char pattern = 0x5A;
	char antiPattern = 0xA5;
	unsigned int i;	
      
	/* write all the tested unit to 0x5A */
	for( i = 0x0; (i<<i) < RamSize; i++ )
	{		
		for( offset = 0x1; offset < RamSize; offset <<= 1 )
		{
	           if ( (offset+(1<<i))> RamSize )
	                 break;		
                   RamStrtAdr[offset+(1<<i)] = pattern;
                }
		/* write the tested unit to 0xA5A5A5A5 */
		   RamStrtAdr[1<<i] = antiPattern;
		                   
		for( offset = 0x1; offset < RamSize; offset <<= 1 )
		{
			if( (RamStrtAdr[offset+(1<<i)] != pattern) && ((offset+(1<<i))< RamSize ) )
				return ERROR;
		}		
	}
