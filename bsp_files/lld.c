...
/* Perform Polling Operation */
  ...
  {
    polling_counter --;
    dev_status = lld_StatusGet(base_addr, offset , (polling_type == LLD_P_POLL_WRT_BUF_PGM));
  }
...
{       
  ...
  taskDelay(50);/*Sector Erase Operation  type time is 0.5sec , max is 3.5sec*/
...
unsigned int lld_GetManufactureId
(
FLASHDATA * base_addr   /* device base address in system */
)
{
  ...
  FLASH_WR(base_addr, LLD_UNLOCK_ADDR1, NOR_AUTOSELECT_CMD);
  for(i = 0; i < 10; i++);  
  id  = (unsigned short int)(FLASH_RD(base_addr, 0x0000) & 0xFFFF) ;
  lld_ResetCmd(base_addr);
  return(id);
}
