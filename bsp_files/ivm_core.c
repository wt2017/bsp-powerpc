...
extern void vme_out_string(unsigned char *stringOut);
...
extern void writePort( unsigned int pins, unsigned int value );
...
extern unsigned int g_ucPinTDO;
...
{
	unsigned short usByteSize;
	unsigned short usBitIndex;
	short usByteIndex;
	unsigned char ucByte;
	unsigned char ucFlipByte;

	if ( a_iDataSize % 8 ) {
		usByteSize = a_iDataSize / 8 + 1;
	}
	else {
		usByteSize = a_iDataSize / 8;
	}

	printf( "(" );
	for ( usByteIndex = usByteSize - 1; usByteIndex >= 0; usByteIndex-- ) {
...
{
	long int iSize = 0;
	signed char cCurrentByte, cIndex;
	...
	while ( ( cOpcode = GetByte() ) >= 0 ) {
		...
		case SHR:
			...
			***************************************************************/
			...
			g_usShiftValue = g_usRepeatLoops * GetByte();
			break;
		case SHL:
		...
		case FREQUENCY:
			...
			***************************************************************/
			...
			g_iFrequency = ispVMDataSize() / 1000;
...
void ispVMData( unsigned char * ByteData )
{
	...
	unsigned char cDataByte = 0, compress = 0;
	...
	if (g_usiDataSize%8>0) {
		size = g_usiDataSize/8 + 1;
	}
	else {
		size = g_usiDataSize/8;
	}
...
void ispVMCalculateCRC32( unsigned char a_ucData ) 
{
	...
	unsigned short usCRCTableEntry;
	...
	usCRCTableEntry = crc_table[ g_usCalculatedCRC & 0xF ];
	g_usCalculatedCRC = ( g_usCalculatedCRC >> 4 ) & 0x0FFF;
	g_usCalculatedCRC = g_usCalculatedCRC ^ usCRCTableEntry ^ crc_table[ ucFlipData & 0xF ];
	usCRCTableEntry = crc_table[ g_usCalculatedCRC & 0xF ];
	g_usCalculatedCRC = ( g_usCalculatedCRC >> 4 ) & 0x0FFF;
	g_usCalculatedCRC = g_usCalculatedCRC ^ usCRCTableEntry ^ crc_table[ ( ucFlipData >> 4 ) & 0xF ];
}
...
{
	signed char cPathIndex, cStateIndex;
...
{
	unsigned short usDataSizeIndex;
	unsigned short usErrorCount = 0;
	unsigned short usLastBitIndex;
	...
	usLastBitIndex = a_usiDataSize - 1;
	...
	*****************************************************************************/
	...
	if ( ucDisplayFlag ) {
		...
		*****************************************************************************/
		...
		for ( usDataSizeIndex = ( ( a_usiDataSize + 7 ) / 8 ); usDataSizeIndex > 0 ; usDataSizeIndex-- ) {
...
{
	unsigned short int usDataSizeIndex;
	unsigned short int usLastBitIndex;
	unsigned short int usBufferIndex = 0;
	unsigned short int usOutBitIndex = 0;
	unsigned short int usLVDSIndex;
