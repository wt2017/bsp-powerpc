...
/* Copyright 1990 - 1999 Wind River Systems, Inc. */
...
int ftpdClientSessionTimeout    = 180; /* 180 seconds is typical timeout */ //bjm revise
/* locals */
...
LOCAL STATUS ftpdWorkTask
	...
	FOREVER
	    ...
	    if ((select (FD_SETSIZE, &readset, NULL, NULL, &timeOut)) != 0)
		    ...
		    ftpdCmdSend (pSlot, pSlot->cmdSock, 221, 
                messages [MSG_NO_GOOD_BYE], 0, 0, 0, 0, 0, 0);
            ftpdSessionDelete (pSlot);
            return OK;
		    }
	...
	if (strncmp (pBuf, "USER", 4) == 0)
	   ...
	   if ( loginVerifyRtn != (FUNCPTR)NULL )
		...
		FD_ZERO(&writeset);
        FD_SET(pSlot->cmdSock, &writeset);

        if ((select (FD_SETSIZE, NULL, &writeset, NULL, &timeOut)) != 0)
            {
            if ( FD_ISSET(pSlot->cmdSock, &writeset) )
                {
...
LOCAL void ftpdDataStreamSend
    ...
    FOREVER
        {
        FD_ZERO(&writeset);
        FD_SET(netFd, &writeset);

	    if ((select (FD_SETSIZE, NULL, &writeset, NULL, &timeOut)) != 0)
            {
            if ( FD_ISSET(netFd, &writeset) )
                {
                cnt = read (fileFd, pBuf, BUFSIZE);
                retval = write (netFd, pBuf, cnt);
                if ((cnt > 0) && (retval == cnt))
                    {
                    pSlot->byteCount += cnt;
	...
	FOREVER
	    ...
	    if ((select (FD_SETSIZE, &readset, NULL, NULL, &timeOut)) != 0)
            ...
            if ( FD_ISSET(netFd, &readset) )
                {
                if ((cnt = read (netFd, pBuf, BUFSIZE)) <= 0)
            	    break;
                }
            }
        else
...
LOCAL STATUS ftpdCmdSend
    ...
    FD_ZERO(&writeset);
    FD_SET(controlSock, &writeset);

    if ((select (FD_SETSIZE, NULL, &writeset, NULL, &timeOut)) != 0)
        {
        if ( FD_ISSET(controlSock, &writeset) )
            {
