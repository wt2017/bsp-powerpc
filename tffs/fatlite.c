...
--------------------
01d,29jun05,j_b  replace INCLUDE_RELFS_MAIN with global flFileSysSectorStart
01c,17may05,jb3  Fix to work for DosFS after Reliance fix.
01b,13apr05,jb3  Protect the BPB for TFFS, allow Reliance compatability
 ...
 * dosFS shares TFFS sector 0, other file systems may not
 */
int flFileSysSectorStart = 0;   /* 0 for dosFS, 1 for Reliance */
...
{
    return vol.tl.deleteSector(vol.tl.rec,
                               (vol.bootSectorNo + flFileSysSectorStart) +
                                ioreq->irSectorNo, /* protect tffs at sector 0 */
                               ioreq->irSectorCount);
}
...
static FLStatus renameFile(Volume vol, IOreq FAR2 *ioreq)
{
    ...
    if(file.ownerDirCluster == file2.ownerDirCluster)
    {  /* Same directory */
...
static FLStatus joinFile (File *file, IOreq FAR2 *ioreq)
{
    ...
    if(joinedFile.fileSize > offsetInCluster)
    { /* the joined file extends
beyond file's end of file.*/
        ...
        if(offsetInCluster)
        {      /* join in the middle of a cluster.*/
